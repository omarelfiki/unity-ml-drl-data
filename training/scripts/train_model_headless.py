...
def parse_arguments_and_load_config():
    ...
    parser.add_argument("--env", dest="env_path", required=True, help="Path to the build so it can train headless (no graphics)")
...
def run_training_process(config_file, run_id, env_path):
    ...
    try:
        ...
        process = subprocess.Popen(
            ["mlagents-learn", config_file, "--env", env_path, "--run-id", run_id, "--force", "--no-graphics"],
